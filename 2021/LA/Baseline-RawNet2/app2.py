...
# Classify audio
def classify_audio(model, waveform, device, threshold=0.6):
...
def main():
    ...
    st.write("Upload an audio file or record your voice to check if it's real or fake.")
    ...
    if input_method == "Upload Audio":
        ...
        st.write("Record your voice (5 seconds maximum)")
        ...
        if st.button("Start Recording"):
            ...
            status_placeholder.write("Recording...")
            
            # Record audio
            recording = sd.rec(int(duration * sample_rate), samplerate=sample_rate, channels=1)
            sd.wait()  # Wait until recording is finished
            
            status_placeholder.write("Recording finished!")
        ...
        # Analysis button (outside the recording block)
        if st.session_state.recording_path is not None:
            if st.button("Analyze Recorded Audio"):
                try:
                    # Load and process audio
                    waveform = load_audio(st.session_state.recording_path)
                    ...
                    # Clean up temporary file
                    if os.path.exists(st.session_state.recording_path):
