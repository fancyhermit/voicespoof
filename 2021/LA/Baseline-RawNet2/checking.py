...
def load_audio(file_path, sample_rate=16000):
    # Loads audio with librosa and resamples it if needed
    wav, sr = librosa.load(file_path, sr=sample_rate)  # Always resampled to sample_rate
    wavform = torch.tensor(wav, dtype=torch.float32).unsqueeze(0)  # shape: [1, length]
    return wavform
...
def classify_audio(model, waveform, device):
    waveform = preprocess_waveform(waveform).to(device)
    with torch.no_grad():
        output = model(waveform.unsqueeze(0))  # Add batch dim if needed
